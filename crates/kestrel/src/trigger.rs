...
pub async fn fire(store: &Store) -> Result<Vec<Fired>> {
    ...
    fired.extend(following_up(store).await?);

    Ok(fired)
}

async fn following_up(store: &Store) -> Result<Vec<Fired>> {
    let mut tx = store.begin().await?;
    let comments = tx.unfollowed(github::COMMENTED, AT_A_TIME).await?;
    let mut fired = Vec::new();
    let mut opened = Vec::new();

    for event in comments {
        let Some(mut session) = tx.session_for_follow_up(&event).await? else {
            tx.record_follow_up(&event, None).await?;
            continue;
        };

        if session.state == crate::domain::SessionState::Sealed {
            let origin = tx
                .event(
                    session
                        .started_by
        ...
        let message = event.occurrence.message.as_deref().unwrap_or_default();
        let run =
            crate::session::post_in(&mut tx, &session, &event.occurrence.actor, message).await?;
        tx.record_follow_up(&event, Some(&session)).await?;
